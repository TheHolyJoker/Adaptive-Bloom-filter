...
void serialize(std::iostream &fs, serialization_context &context, std::map<Key, Value> &mp) {
...
template<class Key, class Value>
void deserialize(std::iostream &fs, serialization_context &context, std::map<Key, Value> &mp) {
    std::string dummy;
    int size = 0;
    fs >> dummy >> size >> dummy;
    assert(fs.good());
